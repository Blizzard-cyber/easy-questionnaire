<template>
	<div class="home">
		<bg></bg>
		<div class="head">
			<h1 style="letter-spacing:15px">易调研平台大数据可视化展板</h1>
			<div class="weather">
				<span>{{nowTime}}</span>
			...
			<div class="mainbox">
				<ul class="clearfix">
					<li>
						...
						<box>
							<pie ref="e3" class="sy" :data="ageList" title="问卷标签比例"></pie>
							<pie ref="e4" class="sy" :data="genderList" title="用户性别比例"></pie>
							<pie ref="e5" class="sy" :data="jobList" title="模板使用量比例"></pie>
						</box>
					</li>
					<li>
					<div class="bar">
						...
						<div class="barbox2">
						<ul class="clearfix">
							<li class="pulll_left">累计用户(位)</li>
							<li class="pulll_left">累计问卷回收(个)</li>
						</ul>
						</div>
					</div>
					<div class="map">
...
export default {
	...
	data() {
		return {
			...
			dongtailist: [
				...
				{
					id: 2,
					content:'Cyber选择了协助Andi填写问卷1',
					time:'45分钟前',
					type: 1
				},{
					id: 3,
					content:'tao通过Cyber的问卷码填写了问卷1',
					time:'36分钟前',
					type: 2
				},{
					id: 4,
					content:'Gary通过Cyber的问卷码填写了问卷1',
					time:'25分钟前',
					type: 2
				},{
					id: 5,
					content:'Lily通过Cyber的问卷码填写了问卷1',
					time:'13分钟前',
					type: 2
				},{
					id: 6,
					content:'lingyi通过Cyber的问卷码填写了问卷1',
					time:'5分钟前',
					type: 2
				},
			],
	...
	created () {
		...
		.then(res => {
			...
			this.qylist.data[0] = confirm.slice(-11)
			this.qylist.data[1] = suspect.slice(-11)
			this.qylist.data[0]=[5,12,26,68,78,79,60,85,123,128,146,131]
			this.qylist.data[1]=[6,15,19,18,26,29,15,17,36,45,69,78]
			// console.log(this.zslist)
			this.zslist.date = date.slice(-11)
			this.zslist.data[0] = heal.slice(-11)
			this.zslist.data[0]=[0.12, 0.15, 0.25 ,0.69 , 0.15, 0.16, 0.78, 0.25, 0.36, 0.45, 0.69 ,0.78]
